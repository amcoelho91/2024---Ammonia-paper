    ...
    while flag_excel > 0:
        try:
            if flag_excel == 1:
                ...
                book.save(OUTPUT_DIR / f"outputs_{iter}.xls")
            flag_excel = 0
        except:
            flag_excel = 2
            print("")
...
def save_costs(m: ConcreteModel(), h: int, case_nr: int, prices: dict, resources: dict,
    ...
    if case_nr == 1:
        n = 0
        n += 1
        sh1.write(1, n + 1, f_E - f_hy + f_water - f_oxyg - f_ammonia)
        n += 1
        sh1.write(1, n + 1, f_E)
        n += 1
        sh1.write(1, n + 1, f_hy)
        n += 1
        sh1.write(1, n + 1, f_water)
        n += 1
        sh1.write(1, n + 1, f_oxyg)
        n += 1
        sh1.write(1, n + 1, f_ammonia)
        n += 1
    elif case_nr == 2:
        n_case = 13
        n = 0
